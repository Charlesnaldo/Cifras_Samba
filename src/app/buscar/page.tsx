...
export default function BuscarPage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 pt-24 px-6 pb-20">
      <div className="max-w-7xl mx-auto">
        <div className="mb-12 space-y-6">
          <h1 className="text-4xl font-black text-white italic uppercase tracking-tighter">
            Explorar <span className="text-accent">Repertorio</span>
          </h1>

          <div className="relative group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500 group-focus-within:text-accent transition-colors" size={20} />
            <input
              type="text"
              placeholder="Busque por musica ou artista..."
              value={busca}
              onChange={(e) => setBusca(e.target.value)}
              className="w-full bg-zinc-900/50 border border-zinc-800 p-4 pl-12 rounded-2xl text-white outline-none focus:border-accent/50 focus:ring-1 focus:ring-accent/20 transition-all"
            />
            {busca && (
            ...
            {ritmos.map((ritmo) => (
              <button
                ...
                className={`px-6 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-all ${ritmoAtivo === ritmo
                  ? 'bg-accent text-black shadow-[0_0_20px_rgba(16,185,129,0.3)]'
        ...
        {musicasFiltradas.length > 0 ? (
          ...
          <div className="text-center py-20 border border-dashed border-zinc-800 rounded-[3rem]">
            <p className="text-zinc-500 font-medium">Nenhuma musica encontrada para sua busca.</p>
          </div>
        )}
      </div>
    </div>
