  ...
  return (
    <div className="flex items-center gap-3 bg-zinc-900 border border-zinc-800 p-1.5 rounded-2xl shadow-xl">
      ...
      <button 
        ...
        className="p-2 hover:bg-zinc-800 rounded-xl transition-all active:scale-90 text-zinc-400 hover:text-yellow-500"
      ...
      <div className="flex flex-col items-center justify-center px-2 min-w-[45px] border-x border-zinc-800">
        ...
        <span className={`font-mono font-bold text-base leading-none ${isTransposed ? 'text-yellow-500' : 'text-white'}`}>
      ...
      {/* Botão Reset (Aparece apenas se houver transposição) */}
      {isTransposed && (
        <button 
          onClick={() => setSemitones(0)}
          className="ml-1 p-2 text-zinc-500 hover:text-red-400 transition-colors border-l border-zinc-800"
          title="Resetar tom"
        >
          <RotateCcw size={14} />
        </button>
